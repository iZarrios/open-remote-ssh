...
Keep `ssh2` as the remote SSH transport and implement persistent connection sharing downstream in the extension. An on-demand, per-user background broker will own shared connections across VS Code and VSCodium extension-host processes and exit according to the configured persistence policy. Existing `ControlMaster`, `ControlPersist`, and `ControlPath` directives will provide sharing policy and identity, while private IPC remains in an extension-owned location and is not compatible with OpenSSH's mux protocol. This preserves the existing transport while addressing repeated authentication across extension consumers. Integrated terminals and all other native `ssh` processes cannot create, attach to, inspect, or close these shared connections; native OpenSSH interoperability is explicitly out of scope.
...
The broker exits immediately after its final shared transport has closed and its final extension consumer has disconnected. An idle transport retained by a timed or indefinite `ControlPersist` policy keeps the broker alive until that transport expires or is explicitly closed.
...
Sharing activates automatically when the effective SSH configuration contains a supported `ControlMaster` value and valid `ControlPath`; no additional extension setting is required. `Remote-SSH: Manage Shared Connections...` lists active connections and inactive configured hosts whose policy can create a shared connection. Selecting an inactive eligible host opens it in a new remote VSCodium window through the ordinary connection flow; that window starts the broker if needed, performs authentication in VSCodium, and owns the initial lease. Hosts without supported control directives and attach-only (`ControlMaster no`) hosts without an active master remain unavailable from this interface. This direct handoff also makes `ControlPersist no` useful without a temporary or hidden lease. Closing an active master follows OpenSSH's immediate termination behavior and disconnects its leases, but the command palette requires explicit confirmation that names the affected clients; it may also offer a non-OpenSSH convenience action to close after the final lease is released.
